...
WtCtaEngine::WtCtaEngine() : _tm_ticker(NULL) {}
...
void WtCtaEngine::init(WTSVariant *cfg, IBaseDataMgr *bdMgr, WtDtMgr *dataMgr,
                       IHotMgr *hotMgr, EventNotifier *notifier /* = NULL */) {
...
void WtCtaEngine::on_init() {
  ...
  for (auto it = _ctx_map.begin(); it != _ctx_map.end(); it++) {
    ...
    ctx->enum_position(
        [this, ctx, exec_ids](const char *stdCode, double qty) {
          double oldQty = qty;
          bool bFilterd = _filter_mgr.is_filtered_by_strategy(ctx->name(), qty);
          if (!bFilterd) {
            if (!decimal::eq(qty, oldQty)) {
              //输出日志
              WTSLogger::info("[Filters] Target position of {} of strategy {} "
                              "reset by strategy filter: {} -> {}",
            ...
            if (strlen(cInfo._ruletag) > 0) {
              std::string code = _hot_mgr->getCustomRawCode(
                  cInfo._ruletag, cInfo.stdCommID(), _cur_tdate);
              realCode =
                  CodeHelper::rawMonthCodeToStdCode(code.c_str(), cInfo._exchg);
            }

            for (auto &execid : exec_ids)
              _exec_mgr.add_target_to_cache(realCode.c_str(), qty,
                                            execid.c_str());
          } else {
            //输出日志
            WTSLogger::info("[Filters] Target position of {} of strategy {} "
                            "ignored by strategy filter",
                            stdCode, ctx->name());
          }
        },
        true);
  ...
  if (!decimal::eq(_risk_volscale, 1.0) && _risk_date == _cur_tdate) {
    WTSLogger::log_by_cat("risk", LL_INFO, "Risk scale of portfolio is {:.2f}",
                          _risk_volscale);
    bRiskEnabled = true;
  }
  ...
  //	WTSLogger::info("Portfolio initial position of {} is {}",
  //stdCode.c_str(), pos);
  //}
...
void WtCtaEngine::on_schedule(uint32_t curDate, uint32_t curTime) {
  ...
  if (_pool) {
    ...
    for (auto it = _ctx_map.begin(); it != _ctx_map.end(); it++) {
      CtaContextPtr &ctx = (CtaContextPtr &)it->second;
      _pool->schedule(
          [ctx, curDate, curTime]() { ctx->on_schedule(curDate, curTime); });
    }
    ...
    for (auto it = _ctx_map.begin(); it != _ctx_map.end(); it++) {
      ...
      const auto &exec_ids = _exec_mgr.get_route(ctx->name());
      ctx->enum_position(
          [this, ctx, exec_ids, &target_pos](const char *stdCode, double qty) {
            double oldQty = qty;
            bool bFilterd =
                _filter_mgr.is_filtered_by_strategy(ctx->name(), qty);
            if (!bFilterd) {
              if (!decimal::eq(qty, oldQty)) {
                //输出日志
                WTSLogger::info("[Filters] Target position of {} of strategy "
              ...
              if (strlen(cInfo._ruletag) > 0) {
                std::string code = _hot_mgr->getCustomRawCode(
                    cInfo._ruletag, cInfo.stdCommID(), _cur_tdate);
                realCode = CodeHelper::rawMonthCodeToStdCode(code.c_str(),
                                                             cInfo._exchg);
              }
  ...
  bool bRiskEnabled = false;
  if (!decimal::eq(_risk_volscale, 1.0) && _risk_date == _cur_tdate) {
    WTSLogger::log_by_cat("risk", LL_INFO,
                          "Risk scale of strategy group is {:.2f}",
                          _risk_volscale);
    bRiskEnabled = true;
  }
  ...
  for (auto &m : _pos_map) {
    ...
    if (target_pos.find(stdCode) == target_pos.end()) {
      if (!decimal::eq(m.second->_volume, 0)) {
        ...
        WTSLogger::error(
            "Instrument {} not in target positions, setup to 0 automatically",
            stdCode.c_str());
      }
...
void WtCtaEngine::handle_pos_change(const char *straName, const char *stdCode,
                                    double diffPos) {
  //这里是持仓增量,所以不用处理未过滤的情况,因为增量情况下,不会改变目标diffQty
  if (_filter_mgr.is_filtered_by_strategy(straName, diffPos, true)) {
    //输出日志
    WTSLogger::info("[Filters] Target position of {} of strategy {} ignored by "
                    "strategy filter",
                    stdCode, straName);
    return;
  }
  ...
  for (auto &execid : exec_ids)
    _exec_mgr.handle_pos_change(realCode.c_str(), targetPos, diffPos,
                                execid.c_str());
}
    ...
    for (auto it = sids.begin(); it != sids.end(); it++) {
      ...
      if (cit != _ctx_map.end()) {
        ...
        if (opt == 0) {
          ...
          if (_pool) {
            _pool->schedule(
                [ctx, stdCode, curTick]() { ctx->on_tick(stdCode, curTick); });
          } else
            ctx->on_tick(stdCode, curTick);
        } else {
          std::string wCode = stdCode;
          wCode =
              fmt::format("{}{}", stdCode, opt == 1 ? SUFFIX_QFQ : SUFFIX_HFQ);
          if (opt == 1) {
            if (_pool) {
          ...
          } else //(opt == 2)
          {
            if (adjTick == nullptr) {
              adjTick =
                  WTSTickData::create(curTick->getTickStruct());
              assert(adjTick);
...
void WtCtaEngine::on_bar(const char *stdCode, const char *period,
  ...
  WTSLogger::info("KBar [{}] @ {} closed", key,
                  period[0] == 'd' ? newBar->date : newBar->time);
}

bool WtCtaEngine::isInTrading() { return _tm_ticker->is_in_trading(); }
...
WTSSessionInfo *WtCtaEngine::get_sess_info(const char *stdCode) {
  CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *cInfo =
      _base_data_mgr->getCommodity(codeInfo._exchg, codeInfo._product);
  if (cInfo == NULL)
    return NULL;
...
void WtCtaEngine::notify_chart_marker(uint64_t time, const char *straId,
                                      double price, const char *icon,
                                      const char *tag) {
  if (_notifier)
    _notifier->notify_chart_marker(time, straId, price, icon, tag);
}

void WtCtaEngine::notify_chart_index(uint64_t time, const char *straId,
                                     const char *idxName, const char *lineName,
                                     double val) {
  if (_notifier)
    _notifier->notify_chart_index(time, straId, idxName, lineName, val);
}

void WtCtaEngine::notify_trade(const char *straId, const char *stdCode,
                               bool isLong, bool isOpen, uint64_t curTime,
                               double price, const char *userTag) {
  if (_notifier)
    _notifier->notify_trade(straId, stdCode, isLong, isOpen, curTime, price,
                            userTag);
}